...
type DropdownType = "filter" | "sort" | "profile" | "notification"; // 드롭다운 용도
...
interface DropdownProps {
  ...
  footer?: React.ReactNode; // profile 전용
}
// 드롭다운 타입 별 사이즈
const typeSizeMap: Record<DropdownType, Record<DropdownSize, string>> = {
  ...
  sort: {
    sm: "w-[105px] py-[6px] pl-[8px] pr-[6px] text-[12px]", // 정렬 작은 드롭다운
    md: "w-[120px] py-[8px] px-[10px] text-[14px]", // 정렬 큰 드롭다운
  },
  profile: {
        ...
        {options.map((opt, idx) => (
          <li
            ...
            className={clsx(
              ...
              type === "notification" && idx !== 0 && "border-t border-[#E5E7EB]", // 알림일 때만 위에 border
